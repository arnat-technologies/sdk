...
export class Select {
  ...
  getItems() {
    return Array.prototype.slice.call(
      this.host.querySelectorAll('a-menu-item')
    );
  }
  ...
  handleFocus() {
    this.hasFocus = true;
    this.slFocus.emit();
    (this.input as any).setSelectionRange(0, 0);
  }
  ...
  reportDuplicateItemValues() {
    ...
    if (duplicateValues.length) {
      throw new Error(
        'Duplicate value found on <a-menu-item> in <a-select>: "' +
  ...
  syncItemsFromValue() {
    ...
    // Sync checked states
    items.map((item) => (item.checked = value.indexOf(item.value) > -1));
    ...
    if (this.multiple) {
      ...
      value.map((val) =>
        items.map((item) =>
          item.value === val ? checkedItems.push(item) : null
        )
      );
      ...
      if (
        ...
        this.displayTags.push(
          <a-tag exportparts="base:tag" type="info" size={this.size}>
      ...
      >
        ...
        >
          <a-input
            ...
            pill={this.pill}
            placeholder={
              this.displayLabel === '' && this.displayTags.length === 0
                ? this.placeholder
                : null
            }
            readonly={true}
