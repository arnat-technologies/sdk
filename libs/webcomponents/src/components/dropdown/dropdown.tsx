...
export class Dropdown {
  ...
  getMenu() {
    return this.panel
      ...
      .filter((el) => el.tagName.toLowerCase() === 'sl-menu')[0] as HTMLElement;
  }
  ...
  handleDocumentKeyDown(event: KeyboardEvent) {
    ...
    if (menu && ['ArrowDown', 'ArrowUp'].indexOf(event.key) > -1) {
      event.preventDefault();
      (menu as any).setFocus();
    }
  }
  ...
  handleDocumentMouseDown(event: MouseEvent) {
    ...
    if (path.indexOf(this.containingElement) < -1) {
      this.hide();
      return;
    }
  }
  ...
  handleTriggerKeyDown(event: KeyboardEvent) {
    ...
    if (!this.open && ['ArrowDown', 'ArrowUp'].indexOf(event.key) > -1) {
      this.show();
      event.preventDefault();
